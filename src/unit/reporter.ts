...
`
  ...
  private renderCase(c: XunitCase): string {
    return `  <testcase name="${c.name}" ${this.classnameAttr(c)}time="0">${c.failures.reduce(
      (a, f) => a + '\n' + this.renderFailure(f),
      ''
    )}${this.newlineIfHasItems(c.failures)}</testcase>`
  }
  ...
  private renderFailure(f: XunitFailure): string {
    return `    <${f.type} message="${f.message}" type="${
      f.type === 'failure' ? 'TestFailure' : 'GrammarTestError'
    }">${this.escapedXml(f.body)}</${f.type}>`
  }
